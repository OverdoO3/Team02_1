	...
	DirectX::XMFLOAT3& rayDir)
{
	...
	float localX = mouse.x - scenePos.x;
	float localY = mouse.y - scenePos.y;

	if (localX < 0 || localY < 0 ||
		localX > sceneSize.x || localY > sceneSize.y)
	{
		return false;
	}

	float u = localX / sceneSize.x;
	float v = localY / sceneSize.y;
