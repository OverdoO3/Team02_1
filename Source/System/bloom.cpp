...
bloom::bloom(ID3D11Device* device, uint32_t width, uint32_t height)
{
	...
	// GpuResourceUtilsの静的メソッドを使用した書き方
...
void bloom::make(ID3D11DeviceContext* immediate_context, ID3D11ShaderResourceView* color_map)
{
	...
	immediate_context->PSGetConstantBuffers(9, 1, cached_constant_buffer.GetAddressOf());
	...
	ID3D11SamplerState* samplers[] = { sampler_state.Get(), sampler_state.Get() };
	immediate_context->PSSetSamplers(0, 2, samplers);
	...
	//Restore  states
	immediate_context->PSSetConstantBuffers(9, 1, cached_constant_buffer.GetAddressOf());
