...
void ShapeRenderer::DrawLine(const RenderContext& rc, const DirectX::XMFLOAT3& start, const DirectX::XMFLOAT3& end, const DirectX::XMFLOAT4& color) const
{
	...
	// Z軸を基準に回転作る（lineMeshがZ方向向いてる前提）
	XMVECTOR zAxis = XMVectorSet(0, 0, 1, 0);
	XMVECTOR axis = XMVector3Cross(zAxis, dir);
	float angle = acosf(XMVectorGetX(XMVector3Dot(zAxis, dir)));
	...
	// スケール（長さ）
	XMMATRIX S = XMMatrixScaling(1.0f, 1.0f, length);

	// 平行移動（start位置）
	XMMATRIX T = XMMatrixTranslation(start.x, start.y, start.z);
