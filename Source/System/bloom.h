...
public:
	...
	void draw(ID3D11DeviceContext* immediate_context,
		...
		UINT num_views = 1,
		ID3D11PixelShader* replaced_ps = nullptr)
	{
		bit_block_transfer->blit(immediate_context, srvs, 0, num_views, replaced_ps);
	}

public:
	float bloom_extraction_threshold = 0.5f;
	float bloom_intensity = 2.0f;
private:
